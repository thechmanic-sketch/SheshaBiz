...
fun CreateQuoteScreen(
    ...
    Column(
        ...
        ScreenHeader(
            title = if (isEditing) stringResource(R.string.edit_quote_title) else stringResource(R.string.create_quote_title),
            onBack = onBack
        )
        ...
        Column(modifier = Modifier.padding(horizontal = 20.dp)) {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                QQDateField(
                    ...
                    millis = state.quoteDate,
                    onChange = viewModel::onQuoteDateChange,
                    modifier = Modifier.weight(1f)
                )
                QQDateField(
                    label = stringResource(R.string.valid_until),
                    millis = state.validUntil,
                    onChange = viewModel::onValidUntilChange,
                    modifier = Modifier.weight(1f)
                )
            }
